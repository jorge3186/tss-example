...
const srcFiles: string[] = ['./data/**'];
const manifestManager = new ManifestManager({
  appName: (config as any).appName,
  rootPath: appRoot,
  watchOnlySourceFiles: (srcFiles as never[]),
});
...
const startDisconnectedServer = (manifest) => {
  ...
  const layoutService = createDisconnectedLayoutService({
    manifest,
    manifestLanguageChangeCallback: manifestManager.getManifest as any,
  });
...
if ((webpackEnv as any).content && (webpackEnv as any).content === 'disconnected') {
  startPromise = manifestManager
    .getManifest((config as any).language)
